...
error:
	...
	# load template pointer and width
	lw $t0, 0($a1)
	lw $t5, 4($a0) # width
	...
	#set i = 0
	li $t1, 0
	li $t3, 7 #template width and length
	...
	for_1_t:
		...
		exit_2_t:
			...
			#send the pointer to the next row
			lw $s1, 4($sp) #reinitialize pointer at initial pointer
			...
			mul $t4, $t1, $t5 #how many rows we're at based on i
			add $s1, $s1, $t4
