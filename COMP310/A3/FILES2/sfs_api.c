...
const int max_ints_in_block = 256;

const int max_dir_len = 500;
const int num_of_inodes = 501;
const int num_of_inodes_per_block = 18;

const int max_fname_size = 16; // Maximum filename size
const int max_number_of_open_files = 300; // Maximum number of open files

int data_block_start;
int num_of_data_blocks;

const int num_files_per_dir_block = (int) block_size / 24; // 16 characters + 2 int = 24 bytes
...
void mksfs(int fresh) {
    ...
    if (fresh) init_fresh_disk("sfs_disk.disk", block_size, max_blocks);
    else init_disk("sfs_disk.disk", block_size, max_blocks);
    init_bitmap(fresh);
...
int sfs_fwrite(int fileID, const char *buf, int length) {
    ...
    while (length > 0) {
        ...
        int inode_link_index = rw_ptr / block_size;
        ...
        if (block_size - block_offset < length) {
            ...
            write_length = block_size - block_offset;
        } else {
        ...
        if (inode_link_index > 11) {
            ...
            if (inode.indirect == -1) { // No space left on disk
                break;
            }
            // Write the data
            ...
            if (inode.direct[inode_link_index] == -1) { // No space left on disk
                break;
            }
            // Write the data
        ...
        rw_ptr += write_length;
        buf += write_length;
        if (rw_ptr > inode.size) inode.size = rw_ptr;
...
int sfs_fread(int fileID, char *buf, int length) {
    ...
    while (length > 0) {
        ...
        // If the remaining space in the block is less than the length of the buffer, write to the remaining space
        if (file_size - rw_ptr < read_length) read_length = file_size - rw_ptr;
        // If the remaining space in the block is greater than the length of the buffer, write the rest of the buffer
        if (block_size - block_offset < read_length) read_length = block_size - block_offset;
...
int sfs_fseek(int fileID, int loc) {
    ...
    if (loc < 0) {
        ...
        return -1;
    } else if (fileID < 0 || fileID >= max_number_of_open_files) {
        printf("Invalid location\n");
        return -1;
    }
    if (fdt[fileID].valid == 0) {
...
int sfs_remove(char *file) {
    ...
    for (int i = 0; i < max_dir_len; i++) {
        if (strcmp(dir_cache[i].name, file) == 0) {
            ...
            dir_cache[i].valid = 0;
            // Remove the file from the file descriptor table
            for (int j = 0; j < max_number_of_open_files; j++) {
