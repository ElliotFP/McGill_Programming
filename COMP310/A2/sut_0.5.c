...
void sut_yield()
{
    ...
    struct queue_entry *entry = queue_new_node(c_tcb);
    pthread_mutex_lock(&c_lock);
    queue_insert_tail(&ready_queue, entry);
    pthread_mutex_unlock(&c_lock);

    // Save context of current user thread and switch back to parent thread
    swapcontext(c_tcb->context, c_manager);
}
...
void preIO()
{
    ...
    pthread_mutex_lock(&i_lock);
    i_tcb = c_tcb;
    struct queue_entry *entry = queue_new_node(i_tcb);
    queue_insert_tail(&wait_queue, entry);
    pthread_mutex_unlock(&i_lock);
    swapcontext(i_tcb->context, c_manager);
}
...
void postIO()
{
     ...
     */
    // Add current user thread to ready queue
    struct queue_entry *entry2 = queue_new_node(i_tcb);
    pthread_mutex_unlock(&i_lock);
    pthread_mutex_lock(&c_lock);
    queue_insert_tail(&ready_queue, entry2);
    pthread_mutex_unlock(&c_lock);
    swapcontext(i_tcb->context, parent_i);
}
